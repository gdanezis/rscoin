...
if __name__ == "__main__":
    ...
    plt.plot(X, m_r1, label="Pay (initial)", linestyle="dashed")    
    v = v_r1 / (len(cnt_r1)**0.5)
    plt.fill_between(x=X, y1=m_r1 - v, y2=m_r1+v, alpha=0.2, color="g")

    plt.plot(X, m_r2, label="Pay (subsequent)", linestyle="dashdot")
    ...
    plt.savefig(join(directory, "Throughput.pdf"))
